...
class SubQuestionQueryEngine(BaseQueryEngine):
    ...
    def from_defaults(
        ...
        if service_context is not None:
            ...
            # TODO: solve error. 
            # AttributeError: 'VectorDBRetriever' object has no attribute 'callback_manager'
    ...
    async def _aquery_subq(
        ...
        try:
            with self.callback_manager.event(
                ...
                sources = response.source_nodes if self.get_sources else []
                qa_pair = SubQuestionAnswerPair(
                    sub_q=sub_q, answer=response_text, sources=sources
                )
